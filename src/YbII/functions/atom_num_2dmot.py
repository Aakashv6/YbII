...
def getS0(p, w1=0.329e-2, w2=0.866e-2):
    return 2 * p / np.pi / w1 / w2 * (0.915 + 0.915 ** 3) / const.Isat_1s0_1p1
...
def rho(f, p):
    return getS0(p) / 2 / (1 + getS0(p) + (2 * (f - const.w0_1s0_1p1_32 + 2 * np.pi * 100e6) / const.gamma_1s0_1p1) ** 2)
...
# p is the measured power per beam before any VP
def plotMOTNumber(img, img_bg, f_data, t_exp, df, p, d0, fit_override=None, param_init=None, constraints=None, plot_save=True, save_dir=r"C:\Users\YbII Cobra\Desktop\0509analysis"):
...
def var_extract(dir, keywords=[["exp", 1e-3], ["d0", 1e-3]], save_params=True):
    ...
    if save_params:
        np.savetxt(r"C:\Users\YbII Cobra\Desktop\0509analysis" + "\exp_params.csv", exp_params, delimiter=",", fmt='%s')

var_extract(r"C:\Users\YbII Cobra\Desktop\0509analysis", [['t', 1e-6]])

def fit_data(dir, t_exp, df, p, d0, fit_override=None, param_init=None, constraints=None, plot_save=True, save_data=False):

    exp_params = np.loadtxt(r"C:\Users\YbII Cobra\Desktop\0509analysis" + "\exp_params.csv", dtype=str, delimiter=",")
    ...
    for f in range(exp_params.shape[0]):
        if exp_params.shape == (2,):
            f_data = exp_params[0]
            f_bg = exp_params[0][:-4] + "_bg.bmp"
        else:
        ...
        img_res, x0, y0, wx, wy, bgx, bgy, x_data, x_fit, y_data, y_fit, atom_num = plotMOTNumber(img_data, img_bg, f_data, t_exp, df, p, 395e-3, fit_override, param_init, constraints, plot_save)
    ...
    if exp_params.shape == (2,):
        exp_params = np.concatenate((exp_params, atom_nums.flatten()))
        exp_params = np.concatenate((exp_params, f_data_list.flatten()))
    else:
        exp_params = np.concatenate((exp_params, atom_nums.T), axis=1)
        exp_params = np.concatenate((exp_params, f_data_list.T), axis=1)
    exp_params = np.savetxt(r"C:\Users\YbII Cobra\Desktop\0509analysis" + "\exp_params.csv", exp_params, delimiter=",", fmt='%s')

fit_data(r"C:\Users\YbII Cobra\Desktop\0509analysis/", 11970e-6, 2 * np.pi * 751.5270425e12, 46.5e-3, 350e-3)
